...
//createEmployeeCredentials("kv12", 123 , "HiMe","linnane","manager","kevin@gmail.com", "M");//create employee works
...
function createEmployeeCredentials(userName, password, fName, lName, employeeType, email, gender, prefNumShift =5, prefWeekend=0) {
        ...
        conn.query(sql, function (err) {
            ...
            createEmployeeInfo(fName, lName, employeeType, email, gender, prefNumShift, prefWeekend);
        });
}
//this method may be nested within createEmployeeCredentials, if so, update the parameters of said function.
function createEmployeeInfo(fName, lName, employeeType, email, gender, prefNumShift, prefWeekend){
...
function updatePassword(fName, lName, newPass) {
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if (err) throw err;
        if (result.length > 0) {
...
function updateUserName(fName, lName, newUserName) {
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if (err) throw err;
        if (result.length > 0) {
 ...
 */
function updateEmployeeType(fName, lName, updatedType) {
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if (err) throw err;
        if (result.length > 0) {
 ...
 */
function updateEmployeePrefWeekends(fName, lName, preferenceNumber ) {
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if (err) throw err;
        if (result.length > 0) {
 ...
 */
function updateEmployeePrefShifts(fName, lName, preferenceNumber ) {
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if (err) throw err;
        if (result.length > 0) {
 ...
 */
function requestedDayOff(fName, lName, reqStartDate, reqEndDate, reason) {
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if(err) throw err;
        if(result.length > 0)  {
 ...
 */
function adminUpdatePendingRTO(fName, lName, reqStartDate, reqEndDate, adminAnswer) {  //this is glitchy.  Updates ID and doesnt consider the dates being inputted.
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if (err) throw err;
        if (result.length > 0) {
 ...
 */
function rolesTrained(fName, lName, added) { //before use this method, only pass through roles which the user already doesnt have. TO do this, a for loop, get results from select query and add the different ones within the for loop
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if(err) throw err;
        if(result.length > 0)  {
...
function deleteTrainingRole() { //use this if training is a result in the roles Trained
 ...
 */
 function updateAvailability(fName, lName, workableDay, newStartTime, newEndTime, callback) {  //change the days which they are available
        var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
...
function insertAvailabilityForNull(fName, lName, workableDay, startTime, endTime, callback) {
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if (err) throw err;
        if(result.length > 0) {
 ...
 */
function insertScheduleForNull(fName, lName, assignedWorkDay, startWorkTime, endWorkTime, role) { //If the set from the select shwows that they are not working that day, add it otherwise, update their workdays.
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if(err) throw err;
        if(result.length > 0)  {
 ...
 *
 */
function updateSchedule(fName, lName, assignedWorkDayToUpdate, newStartWorkTime, newEndWorkTime, role) {  //change the days which they are available
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
        if(result.length > 0) {
...
function deleteUser(fName, lName ) { //deletes on cascade.  Which means the Database will remove the user from ALL tables this person is in because table is setup like that.
    var sqlUserID = `SELECT id FROM employee_init WHERE lName = '${lName}' AND fName = '${fName}'`;
    conn.query(sqlUserID, function(err, result) {
